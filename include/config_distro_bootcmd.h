...
#define BOOTENV \
	...
	\
        ...
        "fsaddr=0x4FE00000\0"\
        "fdt_addr=0x4FA00000\0"\
        "fdt_file=sun50i-h5-orangepi-prime.dtb\0" \
        "image=Image\0" \
        "initrd=uInitrd\0" \
        "setbootargs=setenv bootargs root=/dev/ram0 rw rootwait console=ttyS0,115200n8 ramdisk_size=64000\0" \
        "mmcloadimage=ext2load mmc ${mmcdev}:${mmcpart} ${loadaddr} /NOVAboot/${image}\0" \
        "mmcloadinitrd=ext2load mmc ${mmcdev}:${mmcpart} ${fsaddr} /NOVAboot/${initrd}\0" \
        "mmcloadfdt=ext2load mmc ${mmcdev}:${mmcpart} ${fdt_addr} /NOVAboot/${fdt_file}\0" \
        "mmcloadbootenv=ext2load mmc ${mmcdev}:${mmcpart} ${loadaddr} /NOVAboot/${bootenv}\0" \
        "distro_bootcmd=run setbootargs; run mmcloadimage;run mmcloadinitrd;run mmcloadfdt;booti ${loadaddr} ${fsaddr} ${fdt_addr};\0" \
